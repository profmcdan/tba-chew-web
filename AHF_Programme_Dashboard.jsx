...
const MONTHLY_DATA = [
  ...
  { month:"May", clients:419, deliveries:64, referrals:45, deaths:1 },
];

const DANGER_SIGNS = [
  { sign:"Severe headache / blurred vision", count:142, pct:34 },
  { sign:"Heavy vaginal bleeding",           count:98,  pct:23 },
  { sign:"Facial/hand swelling (oedema)",    count:87,  pct:21 },
...
const Sidebar = ({ active, onNav, role }) => (
  <div style={{ width: 220, background: T.navy, minHeight: "100vh", display: "flex", flexDirection: "column", flexShrink: 0 }}>
    ...
    <div style={{ padding: 16, borderTop: "1px solid rgba(255,255,255,0.08)" }}>
      <div style={{ fontSize: 10, color: "rgba(255,255,255,0.25)", textAlign: "center" }}>Lagos State · 10 LGAs Active<br/>Data current as of 29 May 2026</div>
    </div>
  </div>
);
  ...
  return (
    <div style={{ padding: 28 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(6, 1fr)", gap: 12, marginBottom: 24 }}>
        {[
          { label:"TBAs Active",        value:"219",   trend:+12, icon:"users",  color:T.teal },
          { label:"Clients Registered", value:"2,537", trend:+18, icon:"heart",  color:T.navy },
          { label:"Deliveries Recorded",value:"364",   trend:+22, icon:"baby",   color:T.purple },
          { label:"Referrals This Month",value:"261",  trend:+8,  icon:"refer",  color:T.blue },
          { label:"Referral Completion", value:"91%",  trend:+3,  icon:"check",  color:T.green },
          { label:"Maternal Deaths",     value:"18",   trend:-24, icon:"skull",  color:T.red },
        ].map((k, i) => (
      ...
      {/* Alerts */}
      <Card>
        <CardHeader title="Actionable Alerts" subtitle="Items requiring programme-level attention" icon="alert" iconColor={T.red} />
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {[
            { level:"CRITICAL", title:"3 LGAs with <50% training completion", body:"Ajeromi, Kosofe, Alimosho — immediate intervention required. 41 TBAs untrained.", color: T.red, bg: T.redLight },
            { level:"HIGH",     title:"High maternal death rate in Alimosho", body:"4 deaths this period vs programme average of 1.8. CHEW supervisor review needed.", color: T.orange, bg: T.orangeLight },
            { level:"MEDIUM",   title:"22 Mama Bags due for refresh", body:"Bags due in next 30 days across 6 LGAs. Coordinate with logistics team.", color: T.amber, bg: T.amberLight },
          ].map((a, i) => (
            <div key={i} style={{ background: a.bg, border: `1.5px solid ${a.color}30`, borderRadius: 12, padding: "14px 16px" }}>
              ...
              <button style={{ marginTop: 10, background: "none", border: `1px solid ${a.color}`, borderRadius: 8, padding: "5px 12px", fontSize: 11, fontWeight: 700, color: a.color, cursor: "pointer", fontFamily: "'DM Sans', sans-serif" }}>Take Action</button>
            </div>
          ))}
        </div>
      </Card>
...
const LGABreakdown = () => {
  ...
  const sorted = [...LGA_DATA].sort((a, b) => b[sortKey] - a[sortKey]);
  const max = v => Math.max(...LGA_DATA.map(d => d[v]));
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 20 }}>
        ...
        <Card>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
            {[...LGA_DATA].sort((a,b)=>b.tbas-a.tbas).map((l, i) => (
...
const MaternalHealth = () => (
  <div style={{ padding: 28 }}>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 20 }}>
      <Chip label="Maternal Deaths (This Period)" value="18" trend={-24} color={T.red} />
      <Chip label="Maternal Mortality Ratio (est.)" value="1,041" trend={-18} color={T.orange} />
      <Chip label="ANC Coverage (≥4 contacts)" value="67%" trend={+11} color={T.teal} />
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 16 }}>
      ...
      <Card>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          {[["Pre-eclampsia / hypertension","48",T.red],["Anaemia (severe)","31",T.orange],["Previous C-section","67",T.amber],["Sickle cell","24",T.purple],["Multiple pregnancy","13",T.blue]].map(([l,v,c]) => (
...
const ReferralsOutcomes = () => (
  <div style={{ padding: 28 }}>
    <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 20 }}>
      <Chip label="Total Referrals (This Period)" value="261" trend={+8} color={T.blue} />
      <Chip label="Emergency Referrals" value="41" trend={-12} color={T.red} />
    ...
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
      <Card>
        <CardHeader title="Referral Status — Live" subtitle="Updated in real time" icon="info" iconColor={T.teal}
          action={<div style={{ display: "flex", alignItems: "center", gap: 6 }}><div style={{ width: 8, height: 8, borderRadius: 4, background: T.green }} /><span style={{ fontSize: 11, color: T.green, fontWeight: 700 }}>LIVE</span></div>} />
        {[["INITIATED","4",T.orange],["EN ROUTE","2",T.blue],["RECEIVED","8",T.teal],["OUTCOME RECORDED","247",T.green]].map(([l,v,c]) => (
...
const TBAActivity = () => (
  <div style={{ padding: 28 }}>
    ...
    <Card style={{ marginBottom: 16 }}>
      <CardHeader title="TBA Activity Table" subtitle="All TBAs across 10 LGAs" icon="users" iconColor={T.teal}
...
const DataExport = ({ role }) => {
  ...
  return (
    <div style={{ padding: 28 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        {[
          { id:"state-summary", title:"State-Level Summary", desc:"Aggregate counts across all LGAs: TBAs active, clients registered, deliveries, referrals, maternal deaths. Ministry-grade format.", format:"CSV + DHIS2 JSON", size:"~4KB", time:"Last: 28 May 2026" },
          { id:"lga-breakdown", title:"LGA Breakdown Report", desc:"Same metrics broken down by all 10 active LGAs. Includes risk status, training completion, Mama Bag status.", format:"CSV", size:"~22KB", time:"Last: 28 May 2026" },
          { id:"maternal-outcomes", title:"Maternal Outcomes Dataset", desc:"Delivery outcomes, danger sign frequency, referral outcomes, maternal mortality trend. Quarterly format compatible with HMIS.", format:"CSV + PDF Summary", size:"~38KB", time:"Last: 1 May 2026" },
          { id:"tba-register", title:"TBA Programme Register", desc:"All registered TBAs with LSTMB status, certification date, training completion, and LGA. No client data included.", format:"CSV", size:"~14KB", time:"Last: 28 May 2026" },
          { id:"dhis2-push", title:"DHIS2 Direct Push", desc:"Push current period's anonymised data directly to the Lagos State DHIS2 instance via API. Requires MoH API credentials.", format:"DHIS2 API", size:"Auto", time:"Last push: 1 May 2026" },
...
export default function Dashboard() {
  ...
  const TITLES = {
    overview:  ["State Overview", "Lagos State · All 10 LGAs · May 2026"],
      ...
      <div style={{ display: "flex", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
        ...
        <div style={{ flex: 1, display: "flex", flexDirection: "column", overflow: "auto" }}>
          ...
          <div style={{ padding: "12px 28px", background: T.white, borderTop: `1px solid ${T.grey100}`, display: "flex", justifyContent: "space-between", fontSize: 11, color: T.grey400 }}>
            <span>Access Heart Foundation · Kairoi Intel · Programme Manager & Ministry Dashboard · v2.0 · May 2026</span>
